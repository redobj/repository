...
public class TemperatureSpider implements PageProcessor{
    @Autowired
    private Site site;
    @Value("${spider.time}")
    private int time;
    ...
    private void analysis(Document document) {
        String tem_regex = "^\\S+\\u6e29\\u5ea6\\S+$";
        String numRegex = "[0-9.]+[\\u2103\\u5ea6-]";
        Elements elements = document.getAllElements();
        Pattern pattern = Pattern.compile(numRegex);
        Matcher matcher;
        Double temp;
        for (Element element : elements) {
            if(element.text().matches(tem_regex)) {
                matcher = pattern.matcher(element.text());
                while(matcher.find()) {
                    temp = Double.parseDouble(matcher.group().replaceAll("[\\u2103\\u5ea6-]",""));
    ...
    private Msg getResult() {
        ...
        System.out.println("--------- "+keyword+" 温度获取完毕---------");
        System.out.println("用时 "+time/1000 +"s");
        System.out.println("有效数据" + res.size() + "条");
        System.out.println("最佳温度 " + avg +"℃");
        return Msg.success().add("time",time/1000).add("dataSize",res.size()).add("temperature",avg);
    }
    ...
    public Msg spiderRun(String goodsname){
        ...
        spider.start();
        System.out.println("---------系统正在获取 "+goodsname+" 的最佳保存温度，请等待约 " + (this.getTime())/1000+" s---------");
        try {
